...
// Ambil data riwayat transaksi keluar/masuk
$query = "SELECT t.*, k.*, tr.*, 
          ...
          ORDER BY t.waktu_masuk DESC LIMIT 100";
...
<head>
    ...
    <title>Laporan Parkir - Jogja Bay</title>
    ...
    <aside class="w-full md:w-64 glass-panel border-r border-white/10 p-4 md:p-6 flex flex-col justify-between shrink-0">
        <div class="space-y-6">
            ...
            <nav class="flex md:flex-col gap-2 text-xs">
                ...
                <a href="laporan.php" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-amber-500/20 text-amber-300 border border-amber-500/30 font-bold transition"><i class="fa-solid fa-file-lines w-5 text-center"></i> Laporan Parkir</a>
                <a href="data_kendaraan.php" class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-300 hover:bg-slate-800/60 transition"><i class="fa-solid fa-car-side w-5 text-center"></i> Data Kendaraan</a>
            </nav>
        </div>
    ...
    <main class="flex-1 p-4 md:p-8 space-y-6 overflow-y-auto">
        <div class="glass-panel p-6 rounded-2xl border border-white/10 space-y-4">
            <div class="flex justify-between items-center">
                <h2 class="text-xs font-bold uppercase tracking-wider text-amber-400"><i class="fa-solid fa-file-lines mr-2"></i> Laporan Seluruh Transaksi Parkir</h2>
                <button onclick="window.print()" class="bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs transition"><i class="fa-solid fa-print mr-1"></i> Cetak Laporan</button>
            </div>
                        ...
                        <?php if ($result && mysqli_num_rows($result) > 0): ?>
                            <?php while($row = mysqli_fetch_assoc($result)): ?>
                                <tr>
                                    ...
                                    <td class="py-3 px-3 uppercase text-slate-300"><?= htmlspecialchars($row['jenis_kendaraan']); ?></td>
                                    ...
                                    <td class="py-3 px-3 font-semibold text-emerald-400"><?= $row['biaya_total'] ? 'Rp ' . number_format($row['biaya_total'], 0, ',', '.') : '-'; ?></td>
                                    <td class="py-3 px-3 text-center">
                                        <span class="px-2.5 py-1 rounded-lg text-[10px] font-semibold <?= $row['status'] == 'masuk' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30' : 'bg-slate-800 text-slate-400'; ?>">
                                            <?= ucfirst($row['status']); ?>
                                        </span>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="8" class="py-6 text-center text-slate-400 italic">Belum ada data laporan transaksi.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
